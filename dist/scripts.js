"use strict";
angular.module("educo.utils", ["ngRoute", "ngMaterial"]).factory("appStorage", function () {
    return {
        get: function (n) {
...
}).factory("appPromise", [function () {
    return function (n) {
        var o = Q.defer();
        return n(o), o.promise
    }
}]).factory("appLocation", ["$location", function (n) {
    return n
}]).factory("appWebSocket", [function (n) {
    var o = {
        conn: {}, connect: function () {
            var n = this, o = window.io();
    ...
    return o.connect(), o
}]).factory("appToast", ["$mdToast", function (n) {
    return function (o) {
        var t = n.simple().content(o).action("OK").highlightAction(!1).position("top right");
        n.show(t)
    }
}]).factory("appDialog", ["$mdDialog", function (n) {
    return n
}]).factory("appDesktop", ["$rootScope", function (n) {
    var o = 0, t = 0;
    return {
        notify: function (i) {
            o = void 0 !== i.notificationsCount ? i.notificationsCount : o, t = void 0 !== i.messagesCount ? i.messagesCount : t, n.badges = {messageBadge: t}, window.fluid && (window.fluid.dockBadge = o + t, parseInt(window.fluid.dockBadge) <= 0 ? window.fluid.dockBadge = void 0 : (window.fluid.playSound("Sosumi"), window.fluid.playSound("Purr")))
        }
    }
}]).directive("setFocus", ["$timeout", "$parse", function (n, o) {
    return {
        link: function (t, i, e) {
...
"use strict";
angular.module("educo.system", ["ngRoute", "ngMessages", "ngResource", "angularFileUpload", "educo.utils", "angular-loading-bar", "ngAnimate"]), angular.module("educo.system").factory("tokenHttpInterceptor", ["appStorage", function (e) {
    return {
        request: function (r) {
            return r.headers.Authorization = "Bearer " + e.get("userToken"), r
        }
    }
}]).factory("appSearch", ["$resource", function (e) {
    var r = e("search/:keyword", {}, {query: {isArray: !1}});
    return function (e) {
        var t = r.query({keyword: e}).$promise;
        return t
    }
}]).config(["$httpProvider", "$mdThemingProvider", "cfpLoadingBarProvider", function (e, r, t) {
...
"use strict";
angular.module("educo.settings").controller("SettingsCtrl", ["$scope", "$rootScope", "$routeParams", "$timeout", "appPosts", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", "appUsersSearch", "appSettings", function (t, s, e, a, n, o, r, i, p, c, u, g) {
    g.fetch(function (s) {
        t.systemSettings = s
...
"use strict";
angular.module("educo.settings").config(["$routeProvider", "$locationProvider", function (t, e) {
    t.when("/settings", {
...
"use strict";
angular.module("educo.settings").factory("appSettings", ["$resource", "$rootScope", function (t, e) {
    return {
        cache: {}, single: t("system-settings/"), fetch: function (t) {
            var s = this, n = s.single.get({}, function () {
...
}]).factory("appSettingsValid", ["appSettings", "appLocation", "$rootScope", function (t, e, s) {
    return function () {
...
"use strict";
angular.module("educo.activities", ["educo.system"]);
"use strict";
angular.module("educo.chats", ["educo.system"]);
"use strict";
angular.module("educo.notifications", ["educo.system"]).run(["$rootScope", "appLocation", "appNotification", "appWebSocket", "appNotificationText", function (o, t, n, i, a) {
...
"use strict";
angular.module("educo.posts", ["educo.system"]);
"use strict";
angular.module("educo.streams", ["educo.system"]);
"use strict";
angular.module("educo.users", ["educo.system"]).factory("appAuth", ["$http", "appStorage", function (e, r) {
    return {
        isLoggedIn: function () {
            ...
            var e = r.get("user");
            return e ? angular.fromJson(e) : {name: "unknown"}
        }
    }
}]);
"use strict";
angular.module("educo.activities").controller("ActivitiesCtrl", ["$scope", "$rootScope", "$routeParams", "$timeout", "appPosts", "appActivities", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", function (t, a, e, o, s, i, c, p, r, n, d) {
    t.lastUpdated = 0, t.newActivitiesCount = 0, t.actions = [];
    var u = e.userId, l = i.get({userId: u, timestamp: t.lastUpdated}, function () {
        t.actions = l.res.records ? l.res.records.concat(t.actions) : t.actions, t.lastUpdated = Date.now()
    })
}]);
"use strict";
angular.module("educo.chats").controller("ChatsCtrl", ["$scope", "$rootScope", "$routeParams", "$timeout", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", "appChats", "appDialog", "appDesktop", function (e, a, s, t, o, r, c, n, i, d, h, p) {
    e.chats = [], e.actions = {};
    var g = {}, u = function () {
        var a = 0;
        _.each(e.chats, function (e) {
            a += e.unread
        }), p.notify({messagesCount: a})
    };
    e.message = function (e, a, s) {
        var t;
        s ? (s.unread = 0, u(), t = {chatId: s._id}) : t = {participants: [a._id, o.getUser()._id]};
        var r = new d.single(t);
        r.$save(function (a) {
            var s = a.res.record._id;
            g[a.res.record._id] = a.res.record, h.show({
                controller: ["$scope", "appDialog", function (e, t) {
                    u(), e.messages = a.res.record.messages, e.chatId = s, e.firstTime = !0, e.$on("chatMessage", function (a, s) {
                        ...
                        }), i.conn.emit("markAccessed", {chatId: s.chatId, userId: o.getUser()._id})
                    }), e.hide = function () {
                        t.hide()
                ...
                }], templateUrl: "/modules/chats/views/chat-dialog.html", targetEvent: e
            })["finally"](function () {
                delete g[s]
            })
        })
        ...
        g[s.chatId] || e.updateChats({reload: !0})
    })
}]);
"use strict";
angular.module("educo.notifications").controller("notificationsCtrl", ["$scope", "$rootScope", "appLocation", "appUsers", "appNotification", "appWebSocket", "appNotificationText", "appDesktop", function (i, t, o, n, a, s, c, f) {
    i.notificationShown = !1, i.notificationCount = 0, i.items = [], t.$on("notification", function (t, o) {
        ...
        var o = n.notifications.get({notificationId: t._id}, function () {
        ...
        var t = n.notifications.get({}, function () {
            ...
            }), i.items = t.res.notifications, i.notificationCount = t.res.notifications ? t.res.notifications.length : 0, f.notify({notificationsCount: i.notificationCount})
        })
    }, i.updateNotifications()
}]);
"use strict";
angular.module("educo.posts").controller("PostsCtrl", ["$scope", "$route", "$rootScope", "$routeParams", "$timeout", "appPosts", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", "appUsersSearch", "appPostsFeed", "resolvedFeeds", function (e, t, a, n, s, o, r, d, i, c, l, u, f, p) {
    function m(t) {
        var a = t.config || {};
    ...
    var g = n.hashtag, F = (e.timelinePage = n.userId, e.detailPage = n.postId, e.streamPage = n.streamId);
    g && (e.feedsFilter = "#" + g), e.back = function () {
        history.go(-1)
    }, e.loadMore = function () {
        e.feedPage++, e.lastUpdated = 0, e.feed.push({spacer: !0}), e.updateFeed({append: !0})
    ...
    e.checkMentions = function (t, a) {
        if (t) {
            var n = /@([A-Za-z0-9_]+)$/g, s = t.match(n);
            ...
            angular.element(".mentions-results").css({top: o.offset().top})
        }
    }, e.replaceName = function (t) {
        var a = /@([A-Za-z0-9_]+)$/g, n = angular.element(v);
        n.val(n.val().replace(a, "@" + t) + " "), s(function () {
            n.change(), n.focus(), e.mentionsResults = []
        })
    }, e.create = function (t, a) {
        if (t) {
            var n = new o.single({content: this.content, stream: F});
            n.$save(function (t) {
                t.success ? (t.res = angular.extend(t.res, {creator: r.getUser()}), e.updateFeed(), e.reset()) : (e.failure = !0, d(t.res.message))
            })
        }
    }
}]);
"use strict";
angular.module("educo.streams").controller("StreamsPurposeCtrl", ["$scope", "$rootScope", "$routeParams", "$timeout", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", "appStreams", function (e, t, a, r, s, o, n, u, c, i) {
    var m = a.streamId;
    e.getStream = function (t) {
        var a = i.single.get({streamId: t}, function () {
            e.stream = a.res.record, e.stream.purpose = e.stream.purpose || "Set the stream's purpose here..."
        })
    }, e.updateStreamPurpose = function (t) {
        var a = i.single.get({streamId: m}, function () {
            a = angular.extend(a, e.stream), a.$save(function (e) {
                o("Stream purpose updated.")
            })
        })
    }, m && e.getStream(m)
}]).controller("StreamsCtrl", ["$scope", "$rootScope", "$routeParams", "$timeout", "appAuth", "appToast", "appStorage", "appLocation", "appWebSocket", "appStreams", function (e, t, a, r, s, o, n, u, c, i) {
    e.streams = [], e.actions = {}, e.clearThis = function (t) {
        console.log(e), console.log(t), r(function () {
        }, 2e3)
    }, e.processMoreStreams = function (t) {
        r(function () {
            if ("1" === t)e.createNew(); else var a = i.single.get({streamId: t}, function () {
                a.$subscribe({streamId: t}, function () {
                    e.updateStreams({reload: !0}), o("You have subscribed to the new stream."), u.url("/stream/" + t)
                })
            })
        }, 500)
    }, e.unsubscribe = function (t) {
        var a = t._id, r = i.single.get({streamId: a}, function () {
            r.$unsubscribe({streamId: a}, function () {
                e.updateStreams({reload: !0}), o("You have unsubscribed from that stream.")
            })
        })
    }, e.updateStreams = function (t) {
        t = t || {};
        var a = i.single.get({subscribed: !0}, function () {
        ...
        }), r = i.single.get({unsubscribed: !0}, function () {
        ...
        if (t) {
            var a = new i.single({title: this.newStreamName});
            a.$save(function (t) {
                t.success ? (c.conn.emit("stream", t.res._id), e.actions.createNew = !1, e.updateStreams({reload: !0}), u.url("/stream/" + t.res._id)) : (e.failure = !0, o(t.res.message))
            })
        ...
        o("Woot! There is a new stream available!"), e.updateStreams({reload: !0})
    }), e.updateStreams()
}]);
"use strict";
angular.module("educo.users").controller("ActivationCtrl", ["$rootScope", "$scope", "$routeParams", "appUsers", "appToast", "appStorage", "appLocation", function (e, o, s, r, n, a, t) {
    var i = new r.auth({userId: s.userId, activationCode: s.activationCode});
    i.$save(function (e) {
...
}]).controller("PasswordCtrl", ["$rootScope", "$scope", "$routeParams", "appUsers", "appToast", "appStorage", "appLocation", function (e, o, s, r, n, a, t) {
    var i = new r.auth({userId: s.userId, activationCode: s.activationCode});
    i.$save(function (e) {
...
}]).controller("SearchCtrl", ["$scope", "$routeParams", "$location", "$timeout", "$upload", "appUsers", "appAuth", "appToast", "appUsersSearch", function (e, o, s, r, n, a, t, i, u) {
...
}]).controller("ProfileCtrl", ["$scope", "$routeParams", "$location", "$timeout", "$upload", "appUsers", "appAuth", "appToast", "appPosts", "profileData", "resolvedFeeds", "appPostsFeed", "appLocation", function (e, o, s, r, n, a, t, i, u, c, l, d, p) {
    function f(o) {
        var s = o.config || {};
    ...
    e.editProfile = function () {
        ...
        if (o) {
            ...
            var s = a.single.get({userId: g}, function () {
    ...
    e.getProfile = function () {
        a.single.get({userId: g}).$promise.then(function (e) {
            m(e)
        })
    }, e.noPosting = !0, e.loadMore = function () {
        e.feedPage = e.feedPage || 0, e.feedPage++, e.lastUpdated = 0, e.feed.push({spacer: !0}), e.updateFeed({append: !0})
    }, e.updateFeed = function (s) {
        var s = s || {};
        d.getFeeds(angular.extend(s, o, {limitComments: !0, feedPage: e.feedPage}), function (o) {
            angular.extend(e, o.config), f(o)
        })
    }, e.follow = function () {
        e.alreadyFollowing = "";
        var o = new a.single({userId: g});
        o.$follow({userId: g}, function () {
            r(function () {
                e.getProfile()
            }, 800)
        })
        ...
        e.alreadyFollowing = "";
        var o = new a.single({userId: g});
        o.$unfollow({userId: g}, function (o) {
            r(function () {
                e.getProfile()
            }, 800)
        })
        ...
        var o = e.avatar.pop();
        n.upload({url: "/users/" + g + "/avatar", file: o}).progress(function (e) {
...
}]).controller("InviteCtrl", ["$rootScope", "$scope", "appStorage", "appLocation", "appDialog", "appUsers", "appAuth", "appToast", function (e, o, s, r, n, a, t, i) {
    o.inviteOthers = function (e) {
        n.show({
            controller: ["$scope", "appDialog", function (e, o) {
                e.inviteDone = !1, e.doInvite = function (o) {
                    if (o) {
                        var s = t.getUser()._id, r = new a.single({
                            userId: s,
                            message: e.message,
                            email: e.email,
                            name: e.name
                        });
                        r.$invite({userId: s}, function (o) {
            ...
            }], templateUrl: "/modules/users/views/users-invite-dialog.html", targetEvent: e
        })
    }
}]).controller("LogoutCtrl", ["$rootScope", "appStorage", "appLocation", function (e, o, s) {
    o.remove("userToken"), o.remove("user"), e.$broadcast("loggedOut"), s.url("/login")
}]).controller("LoginCtrl", ["$scope", "$rootScope", "appUsers", "appAuth", "appToast", "appStorage", "appLocation", "appDialog", function (e, o, s, r, n, a, t, i) {
    e.reset = function () {
        ...
        var o = new s.single({userId: e.registeredUserId});
        o.$activate({userId: e.registeredUserId}, function () {
        ...
        i.show({
            controller: ["$scope", "appDialog", function (e, o) {
                e.inviteDone = !1, e.doReset = function (o) {
                    if (o) {
                        var r = new s.single({email: e.email});
                        r.$resetPassword({email: e.email}, function (o) {
            ...
            }], templateUrl: "/modules/users/views/users-pwd-dialog.html", targetEvent: e
        })
    }, e.login = function (o) {
        if (o) {
            var r = new s.auth({email: this.email, password: this.password});
            r.$save(function (o) {
...
}]).controller("UserSheet", ["$scope", "$mdBottomSheet", "$location", "appStorage", function (e, o, s, r) {
    e.items = [{
        name: "Profile", icon: "fa-user", handler: function () {
            s.url("/profile/" + angular.fromJson(r.get("user")).username)
        }
    }, {
        name: "Logout", icon: "fa-sign-out", handler: function () {
            s.url("/logout")
        }
...
"use strict";
angular.module("educo.activities").factory("appActivities", ["$resource", function (e) {
    return e("activities/feed/:userId", {userId: "@_id"})
}]);
"use strict";
angular.module("educo.chats").factory("appChats", ["$resource", function (a) {
    return {
        single: a("chats/:chatId/:action", {chatId: "@_id"}, {
            message: {
                method: "POST",
                params: {action: "message"}
            }, end: {method: "POST", params: {action: "end"}}
        })
    }
}]);
"use strict";
angular.module("educo.notifications").factory("appNotification", ["$resource", "$mdToast", function (t, o) {
    return {
        show: function (t) {
            if (t.message) {
...
}]).factory("appNotificationText", [function () {
    return function (t) {
        if (!t)return {text: ""};
        var o = "", i = t.actor;
...
"use strict";
angular.module("educo.posts").factory("appPosts", ["$resource", function (e) {
    return {
        single: e("posts/:postId/:action", {postId: "@_id"}, {
            like: {method: "POST", params: {action: "like"}},
            unlike: {method: "POST", params: {action: "unlike"}},
            comment: {method: "POST", params: {action: "comment"}},
            likes: {method: "GET", params: {action: "likes"}}
        }), feed: e("posts/"), stream: e("posts/stream/:streamId"), timeline: e("posts/timeline/:userId")
    }
}]).filter("appPostFormat", ["$sce", function (e) {
    return function (t) {
        ...
        var i = [{key: ":)", value: "fa-smile-o"}, {key: ":|", value: "fa-meh-o"}, {
            key: ":(",
            value: "fa-frown-o"
        }, {key: "(y)", value: "fa-thumbs-o-up"}, {key: "(n)", value: "fa-thumbs-o-down"}, {
            key: ":+1",
            value: "fa-thumbs-up"
        }, {key: "(h)", value: "fa-heart"}, {
            key: "(i)",
            value: "fa-lightbulb-o"
        }], s = '<md-inline-list-icon class="yellow fa {{emoticon}}"></md-inline-list-icon>';
        for (var a in i) {
            var r = i[a].key, l = i[a].value;
            t = t.replace(r, s.replace("{{emoticon}}", l))
        }
        return e.trustAsHtml(t)
    }
}]).factory("appPostsFeed", ["appPosts", function (e) {
    return {
        getFeeds: function (t, n) {
            ...
            t = t || {};
            var i = t.userId, s = (t.hashtag, t.postId), a = t.streamId, r = t.passedData, l = t;
            if (i) {
                ...
                var m = e.single.get({postId: s, limitComments: l.limitComments, allowMarking: !0}, function () {
...
}]).directive("awFeedItem", ["appPosts", "appWebSocket", "appAuth", "appDialog", function (e, t, n, o) {
    return {
        templateUrl: "/modules/posts/views/post-single.html", controller: ["$scope", function (t) {
            t.doLike = function (t) {
                ...
                e.single.likes({postId: n._id}, function (e) {
                    o.show({
                        controller: ["$scope", "appDialog", function (t, n) {
                            t.users = e.res.records, t.hide = function () {
                                n.hide()
                            }
                        }], templateUrl: "/modules/users/views/users-dialog.html", targetEvent: t
                    })
                })
            }
        }]
...
"use strict";
angular.module("educo.streams").factory("appStreams", ["$resource", function (s) {
    return {
        single: s("streams/:streamId/:action", {streamId: "@_id"}, {
            subscribe: {
                method: "POST",
...
"use strict";
angular.module("educo.users").factory("appUsers", ["$resource", function (r) {
    return {
        single: r("users/:userId/:action", {userId: "@_id"}, {
            update: {method: "PUT"},
            follow: {method: "POST", params: {action: "follow"}},
            unfollow: {method: "POST", params: {action: "unfollow"}},
        ...
        }), auth: r("users/authenticate"), notifications: r("users/notifications/:notificationId")
    }
}]).factory("appUsersSearch", ["$resource", function (r) {
    var e = r("users/search/:keyword", {}, {query: {isArray: !1}});
    return function (r, a) {
        var o = {keyword: r};
        a && (o.onlyUsernames = !0);
...
"use strict";
angular.module("educo.activities").config(["$routeProvider", "$locationProvider", function (o, i) {
    i.html5Mode(!0)
}]);
"use strict";
angular.module("educo.chats").config(["$routeProvider", "$locationProvider", function (o, r) {
    r.html5Mode(!0)
}]);
"use strict";
function resolvedFeeds(e) {
    return ["$route", "appPostsFeed", function (s, t) {
        var o = Q.defer(), l = angular.extend({limitComments: e.limitComments}, s.current.params);
        return t.getFeeds(l, function (e) {
            o.resolve(e)
        }), o.promise
    }]
}
angular.module("educo.posts").config(["$routeProvider", "$locationProvider", function (e, s) {
    e.when("/feed", {
        templateUrl: "/modules/posts/views/feed.html",
        controller: "PostsCtrl",
        resolve: {resolvedFeeds: resolvedFeeds({limitComments: !0})}
    }).when("/", {
        templateUrl: "/modules/posts/views/feed.html",
        controller: "PostsCtrl",
        resolve: {resolvedFeeds: resolvedFeeds({limitComments: !0})}
        ...
        templateUrl: "/modules/posts/views/feed.html",
        controller: "PostsCtrl",
        resolve: {resolvedFeeds: resolvedFeeds({limitComments: !1})}
    }).when("/feed/:hashtag", {
        templateUrl: "/modules/posts/views/feed.html",
        controller: "PostsCtrl",
        resolve: {resolvedFeeds: resolvedFeeds({limitComments: !0})}
...
"use strict";
angular.module("educo.streams").config(["$routeProvider", "$locationProvider", function (e, r) {
    e.when("/stream/:streamId", {
        templateUrl: "/modules/posts/views/feed.html",
        controller: "PostsCtrl",
        resolve: {
            resolvedFeeds: ["$route", "appPostsFeed", function (e, r) {
                var t = Q.defer(), o = angular.extend({limitComments: !0}, e.current.params);
...
"use strict";
angular.module("educo.users").config(["$routeProvider", "$locationProvider", function (e, r) {
    e.when("/login", {
        ...
        resolve: {
            profileData: ["$route", "appAuth", "appUsers", function (e, r, t) {
                var o = e.current.params, s = o.userId || r.getUser()._id;
                return t.single.get({userId: s}).$promise
            }], resolvedFeeds: ["$route", "appPostsFeed", function (e, r) {
                var t = Q.defer(), o = angular.extend({feedPage: 0}, e.current.params);
                return r.getFeeds(o, function (e) {
                    t.resolve(e)
                }), t.promise
            }]
    ...
    }).when("/me", {templateUrl: "/modules/users/views/profile.html?v", controller: "ProfileCtrl"}), r.html5Mode(!0)
}]);
var app = angular.module("educo", ["educo.system", "educo.users", "educo.posts", "educo.streams", "educo.chats", "educo.activities", "educo.notifications", "educo.settings", "ngMaterial"]);
app.controller("AppCtrl", ["$scope", "$route", "$rootScope", "$mdSidenav", "$mdBottomSheet", "$location", "$timeout", "appLocation", "appAuth", "appWebSocket", "appSettings", "appSettingsValid", "appToast", function (t, e, o, n, a, i, s, r, c, l, u, g, p) {
    ...
    t.$on("$routeChangeSuccess", function () {
        angular.element("*[md-scroll-y]").animate({scrollTop: 0}, 300), n("left").close()
    }), t.$on("loggedIn", function () {
        t.updateLoginStatus(), t.barTitle = "", t.$broadcast("updateNotifications"), l.conn.emit("online", {token: c.getToken()}), c.refreshUser(function (e) {
            t.user = e
        }), d(function () {
        ...
        t.updateLoginStatus(), l.conn.emit("logout", {token: c.getToken()})
    }), l.conn.on("connect", function () {
        c.isLoggedIn() && l.conn.emit("online", {token: c.getToken()})
